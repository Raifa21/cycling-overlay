...
export const loadActivity = (path: string) =>
  invoke<ActivityInfo>("load_activity", { path });
export const loadLayout = (path: string) =>
  invoke<LayoutInfo>("load_layout", { path });
...
export const watchLayout = (path: string) =>
  invoke<void>("watch_layout", { path });
export const unwatchLayout = () => invoke<void>("unwatch_layout");

export const startExport = (args: ExportArgs) =>
  invoke<void>("start_export", { args });
...
export const onExportLog = (
  fn: (p: LogPayload) => void
): Promise<UnlistenFn> =>
  listen<LogPayload>("export-log", (e) => fn(e.payload));
export const onExportDone = (
  fn: (p: DonePayload) => void
): Promise<UnlistenFn> =>
